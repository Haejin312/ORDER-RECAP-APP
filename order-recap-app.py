...
# Revision History: AE(31)=Date, AF(32)=Notes, row 9부터
ROW_REVISION_START = 9
COL_REV_DATE       = 31   # AE
COL_REV_NOTES      = 32   # AF

# Fabric Combination: row 11~14
# Body: A(1)=Combo, C(3)=Code, D(4)=Fiber, G(7)=Construction, H(8)=OZ, I(9)=GSM
# Trim: J(10)=Code, K(11)=Fiber, N(14)=Construction, O(15)=OZ, P(16)=GSM
FABRIC_ROW_START   = 11
FABRIC_COL_COMBO   = 1    # A
FABRIC_COL_BCODE   = 3    # C
FABRIC_COL_BFIBER  = 4    # D
FABRIC_COL_BCONST  = 7    # G
FABRIC_COL_BOZ     = 8    # H
FABRIC_COL_BGSM    = 9    # I
FABRIC_COL_TCODE   = 10   # J
FABRIC_COL_TFIBER  = 11   # K
FABRIC_COL_TCONST  = 14   # N
FABRIC_COL_TOZ     = 15   # O
FABRIC_COL_TGSM    = 16   # P

# 스케치: row 3, R(18)/U(21)/X(24)/AA(27)
SKETCH_ROW  = 3
SKETCH_COLS = {0: 18, 1: 21, 2: 24, 3: 27}
...
# PO 슬롯: 17개, 홀수 열 (3,5,7,...,35)
PO_SLOT_COLS = [3 + i * 2 for i in range(17)]
                    ...
                    'content':[
                        ...
                        },
                        {
                            ...
                            'text':(
                                ...
                                '}\n'
                                'Rules:\n'
                                '- color_code: suffix after hyphen only. K128-BLK→"BLK". Standalone codes as-is.\n'
                                '- fabric_fiber: fiber content only, e.g. "60% COTTON 40% POLYESTER", "100% COTTON"\n'
                                '- fabric_construction: construction type only, e.g. "JERSEY", "KNIT", "FLEECE"\n'
                                '- fabric_oz: weight only, e.g. "6.75"\n'
                                '- dates: MM/DD/YYYY\n'
                                '- fob_prices: per-size FOB prices\n'
...
def build_excel():
    try:
        ...
        # ── File#, Season ──────────────────────────────
        w(ws, ROW_FILE_NUM, 3, file_number)
        if season:
            w(ws, ROW_SEASON, 3, season)
        ...
        for i, fc in enumerate(fabric_combos[:4]):
            ...
            w(ws, row, FABRIC_COL_BFIBER, clean(fc.get('bodyFiber','')))
            w(ws, row, FABRIC_COL_BCONST, clean(fc.get('bodyConst','')))
            w(ws, row, FABRIC_COL_BOZ,    clean(fc.get('bodyOz','')))
            w(ws, row, FABRIC_COL_BGSM,   clean(fc.get('bodyGsm','')))
            w(ws, row, FABRIC_COL_TCODE,  clean(fc.get('trimCode','')))
            w(ws, row, FABRIC_COL_TFIBER, clean(fc.get('trimFiber','')))
            w(ws, row, FABRIC_COL_TCONST, clean(fc.get('trimConst','')))
            w(ws, row, FABRIC_COL_TOZ,    clean(fc.get('trimOz','')))
            w(ws, row, FABRIC_COL_TGSM,   clean(fc.get('trimGsm','')))
        ...
        # ── PO 슬롯 (17개) ────────────────────────────
        for slot, po in enumerate(pos[:17]):
            pcs_col = PO_SLOT_COLS[slot]
            fob_col = pcs_col + 1
            ...
            # Fabric Combo 라벨
            fiber = normalize_fiber(po.get('fabric_fiber','') or po.get('fabric_desc',''))
            combo_label = ''
            for fc in fabric_combos:
                if normalize_fiber(fc.get('bodyFiber','') or fc.get('bodyDesc','')) == fiber:
            ...
            for size, srow in SIZE_ROWS.items():
                ...
                if qty: w(ws, srow, pcs_col, int(qty))
                if fob: w(ws, srow, fob_col, float(fob))

        # ── 스케치 이미지 ─────────────────────────────
        CELL_W_PX = 3 * (int(13*7)+5)
        CELL_H_PX = int(11 * 16.8 * 96 / 72)
        ...
        for idx, style in enumerate(styles_seen[:4]):
            ...
            try:
                ...
                max_h = CELL_H_PX - 12
                if target_w > max_w:
                    target_h = int(target_h*max_w/target_w); target_w = max_w
                if target_h > max_h:
                    target_w = int(target_w*max_h/target_h); target_h = max_h
                img.width  = target_w
                img.height = target_h
                x_off = max(0,(CELL_W_PX-target_w)//2)*PX2EMU
                y_off = max(0,(CELL_H_PX-target_h)//2)*PX2EMU
                col_0 = SKETCH_COLS.get(idx,18)-1
                marker = AnchorMarker(col=col_0, colOff=x_off, row=3, rowOff=y_off)
